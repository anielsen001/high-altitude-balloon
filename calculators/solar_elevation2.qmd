...
times = np.linspace(start_date,end_date,1000,endpoint=False)
...
frames = [ AltAz(obstime=_t, location=pos) for _t in times ]
sunlocs = [get_sun(frames[0].obstime).transform_to(_f) for _f in frames]
alts = [ _sunloc.alt for _sunloc in sunlocs ]

# reformat into an array
altslats = np.zeros([len(times),len(lats)])
for ii in range(len(alts)):
    altslats[ii,:] = alts[ii]
